...
def get_latex(filename):
    # Mục đích: Chuyển đổi một hình ảnh thành các đường cong và điểm sử dụng thư viện potrace 
    # và trả về một danh sách các tọa độ và đường cong ở định dạng LaTeX.
    # Chi tiết:
    # Mở hình ảnh và chuyển đổi nó thành mảng NumPy.
    # Sử dụng potrace để tạo bitmap và truy tìm các đường cong.
    # Tạo các đoạn mã LaTeX cho mỗi đoạn cong hoặc góc của các đường.
    latex = []
...
def get_contours(filename):
        # Mục đích: Tìm và vẽ các đường viền (contour) của hình ảnh và lưu kết quả dưới dạng tệp PNM.
        # Chi tiết:
        # Đọc hình ảnh và chuyển đổi thành ảnh xám.
        # Áp dụng ngưỡng (threshold) để phân biệt các phần tử.
        # Tìm các đường viền và vẽ chúng lên mặt nạ.
        # Lưu mặt nạ đã vẽ thành tệp PNM và trả về kích thước ảnh.
...
def rgb_to_hex(rgb):
    # Đảm bảo chỉ lấy 3 giá trị đầu (R, G, B)
    if isinstance(rgb, int):
        # Ảnh xám, chuyển thành 3 kênh giống nhau
...
def seperate_colors(filename):
    # Mục đích: Tách biệt các màu sắc trong hình ảnh thành các khối màu đen và trắng.
    # Chi tiết:
    # Mở hình ảnh và duyệt qua từng điểm ảnh.
    # So sánh màu sắc của điểm ảnh hiện tại với điểm ảnh trước đó để xác định sự khác biệt.
    # Nếu sự khác biệt đủ lớn, thay đổi màu sắc mới là đen hoặc trắng.
...
def plot_image():
    ...
    return render_template("index.html",  points=points, latex=latex, shape=shape)
